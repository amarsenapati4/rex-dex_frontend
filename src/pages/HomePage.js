...
const HomePage = () => {
  ...
  const getAllCategory = async () => {
    try {
      const { data } = await axios.get(
        "https://ecommerce-backend-us2n.onrender.com/api/v1/category/get-category"
      );
      if (data?.success) {
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get(
        "https://ecommerce-backend-us2n.onrender.com/api/v1/product/product-count"
      );
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
  ...
  useEffect(() => {
    const loadMore = async () => {
      try {
        setLoading(true);
        const { data } = await axios.get(
          `https://ecommerce-backend-us2n.onrender.com/api/v1/product/product-list/${page}`
        );
        setLoading(false);
    ...
    const filterProduct = async () => {
      try {
        const { data } = await axios.post(
          "https://ecommerce-backend-us2n.onrender.com/api/v1/product/product-filters",
          {
            checked,
            radio,
          }
      ...
      <div className="container-fluid row  mt-[3rem] justify-between">
        <div className="flex md:flex-row flex-col gap-4">
          <div className="col-md-2">
            <h4 className="text-center font-bold text-emerald-700 mt-5">
              Filter By Category
            </h4>
            ...
            {/* price filter */}
            <h4 className="text-center mt-4 font-bold text-emerald-700">
              Filter By Price
            </h4>
            <div className="d-flex flex-column">
          ...
          <div className="col-md-9">
            <h1 className="text-center text-3xl mb-2 text-red-600">
              All Products
            </h1>
